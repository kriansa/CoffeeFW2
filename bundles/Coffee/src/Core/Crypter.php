...
class Crypter
{
	...
	public static function encrypt($value)
	{
		...
		$iv = mcrypt_create_iv(static::_iv_size(), $random);

		$pad = static::$block - (Str::length($value) % static::$block);

		$value .= str_repeat(chr($pad), $pad);

		$value = mcrypt_encrypt(static::$cipher, Config::get('system.crypt_key'), $value, static::$mode, $iv);
	...
	public static function decrypt($value)
	{
		...
		$iv = substr($value, 0, static::_iv_size());

		$value = substr($value, static::_iv_size());
		...
		$key = Config::get('system.crypt_key');
		...
		if ($pad and $pad < static::$block)
		{
			...
			{
				throw new CrypterException("Decryption error. Padding is invalid.");
			}
		}
	 ...
	 * @return int
	 */
	protected static function _iv_size()
	{
